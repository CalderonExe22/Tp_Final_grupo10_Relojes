...
class Session
{
    ...
    public function iniciar($nombreUsuario, $pswUsuario)
    {
        ...
        if ($this->activa() && $this->validar($nombreUsuario, $pswUsuario)) {
            ...
            if($_SESSION['usdeshabilitado'] === NULL || $_SESSION['usdeshabilitado'] === "0000-00-00 00:00:00"){
                $resp = true;
            }else{
                $resp = false;
            }
    ...
    public function getUsuarioDeshabilitado()
    {
        //retorna el mail del usuario logueado
        $usDesabi = null;
        if (isset($_SESSION['usdeshabilitado'])) {
            $usDesabi = $_SESSION['usdeshabilitado'];
        }
        return $usDesabi;
    }
    ...
    public function verificarPermisoCabecera(){
        $url=$_SERVER["PHP_SELF"];
        $url=explode("/",$url);
        $ruta="../";
        $ruta.=$url[count($url)-2]."/";
        $ruta.=$url[count($url)-1];
        return $this->verificarPermiso($ruta);
    }
