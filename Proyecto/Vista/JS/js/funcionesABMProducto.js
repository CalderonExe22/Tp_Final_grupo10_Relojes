...
function armarTabla(arreglo) {
    ...
    $('#tablaProductos > tbody').append('<tr class="table-active"><td><a class="" href="ingresarProducto.php"><button id="enviar">agregar producto</button></a></td></tr>');
    ...
    $.each(arreglo, function (index, producto) {
        ...
        $('#tablaProductos > tbody:last-child').append('<tr><td>' + producto.idproducto + '</td><td>' + producto.pronombre + '</td><td>' + producto.imagen + '</td><td>' + producto.procantstock + '</td><td>' + producto.precio + '</td><td hidden>' + producto.imagen + '</td><td><img src="' + producto.prodetalle + '" class="rounded float-start" width="150" height="150"></td><td>' + producto.deshabilitado + '</td><td><a href="#" class="editar me-2"><button class="btn btn-outline-warning"><i class="fa-solid fa-file-pen"></i></button></a>' + boton + '<a href="#" class="eliminar"><button class="btn btn-outline-danger"><i class="fa-solid fa-trash"></i></button></a></td></tr>');
    });
}
...
$(document).ready(function () {
    $('#editarP').submit(function (e) {
        e.preventDefault();
        formdata = new FormData($("#editarP")[0]);
        $.ajax({
            type: "post",
