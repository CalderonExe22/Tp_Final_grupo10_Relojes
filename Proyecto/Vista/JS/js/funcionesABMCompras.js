    ...
    $.each(arreglo, function (index, compra) {
        ...
        switch (compra.estado) {
            ...
            case "aceptada":
                ...
                break;
            case "enviada":
                estadoVista = "<span class='badge rounded-pill text-bg-primary'>Enviada</span>";
                break;
            case "cancelada":
                estadoVista = "<span class='badge rounded-pill text-bg-danger'>Cancelada</span>";
                break;
        }
        $('#tablaCompras > tbody:last-child').append('<tr><td scope="row">' + compra.idcompra + '</td><th style="display:none;" >' + compra.idcompraestado + '</th><td>' + compra.usnombre + '</td><td><a href="#" class="verProductos"><button class="btn btn-outline-info col-8"><i class="fa-solid fa-list-ul mx-2"></i></button></a></td><td>' + estadoVista + '</td><td>' + compra.cofecha + '</td><td>' + compra.finfecha + '</td>' + botones + '</tr>');
    });
}
