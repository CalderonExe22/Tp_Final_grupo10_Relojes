...
function armarTablaProductos(arreglo) {
    ...
    $.each(arreglo, function (index, producto) {
        ...
        if((producto.prodeshabilitado == null || producto.prodeshabilitado == '0000-00-00 00:00:00') && producto.procantstock>0){
            ...
            $('#filaProductos').append('<div class="col-sm-3 pb-3" ><div  class="card" id="listaProductos"><div class="card-body text-center"><div class="card-body text-center"><img src="'+producto.prodetalle+'"class="rounded float-start" width="250" height="250"><h5 class="card-title">'+producto.pronombre+'</h5><p class="card-text"><b>Precio:</b> '+producto.precio+'</p><p class="card-text"><b>Stock Actual: </b>'+producto.procantstock+'</p><div id="agregarBoton'+producto.idproducto+'" class="d-grid gap-2 d-md-block m-auto mb-2"></div></div></div></div></div>');
        }

        if(producto.rol !=null && producto.rol=="cliente"){
            $('#agregarBoton'+producto.idproducto).append('<button class="carrito btn btn-outline-success" type="button" onclick=agregarACarrito('+producto.idproducto+') class="agregarACarrito btn btn-primary btn-sm">Añadir al carrito</button><script src="../JS/js/funcionesCarrito.js"></script>');
